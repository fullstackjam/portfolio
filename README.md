...
- GitHub REST API (profile, languages, latest commits)
...
2. **GitHub APIs** — on cache miss, fetch the profile, languages, and each
   curated project's latest commit ([`src/lib/github.ts`](src/lib/github.ts)),
   then write back to KV.
...
`Hero` · `About` · `Work` (curated projects) · `Contact` — see
...
- `PROJECTS` — curated work shown in the Work section (fully manual, each entry is a `Project` object)
